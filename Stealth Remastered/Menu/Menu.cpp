...
void CMenu::Render()
{
	...
	if (ImGui::Begin("GTS", &bOpen, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar))
	{
		ImGui::BeginGroup();
		ImGui::Dummy({ 0, 2 });
		...
		if (ImGui::TabButton("Unload", &iCurrentTab, 5, { 0, 25 }))
		...
		case 0:
		{
			...
			ImGui::BeginChild("##Aimbot", { 280, 147 }, true, ImGuiWindowFlags_MenuBar);
			{
				...
				if (ImGui::BeginPopupContextItem(0, 0))
				{
					...
					ImGui::Checkbox("Trigger Key Space", &g_Config.g_Aimbot.bTriggerKeyMMB);// ImGui::SameLine(150);
					ImGui::PopItemFlag();
					//ImGui::PopStyleVar();
					Tooltip("Hold Space to Activate Trigger Bot");
				...
				if (!g_Config.g_Aimbot.bAimbot || !g_Config.g_Aimbot.bSilent || !g_Config.g_Aimbot.bTriggerbot)
				{
					ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);
					ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * 0.5f);
				}
				ImGui::Checkbox("GTS SpecialBot", &g_Config.g_Aimbot.bGTS); ImGui::SameLine();
				if (!g_Config.g_Aimbot.bAimbot || !g_Config.g_Aimbot.bSilent || !g_Config.g_Aimbot.bTriggerbot)
				{
					ImGui::PopItemFlag();
					ImGui::PopStyleVar();
					Tooltip("Enable Range, Silent and TriggerBot to use this function!");
				}
				else if (g_Config.g_Aimbot.bAimbot && g_Config.g_Aimbot.bSilent && g_Config.g_Aimbot.bTriggerbot)
				{
					Tooltip("This enables SilentAim+TriggerBot ONLY while holding Space!!!(works with sniper too) shooting normally will NOT activate silent aim");
				}
				ImGui::EndChild();
			}
			...
			ImGui::BeginChild("##Config", { 280, 101 }, true, ImGuiWindowFlags_MenuBar);
			{
				...
				for (auto i : mapWeapons)
				{
					...
					if (ImGui::BeginPopupContextItem(0, 0))
					{
						...
						g_Config.g_Aimbot.bAimbot ? ImGui::TextUnformatted("SRange") : ImGui::TextDisabled("SRange"); ImGui::SameLine(120);
						ImGui::SliderInt("##SRange", &g_Config.g_Aimbot.iAimbotConfig[i.first][SRANGE], 1, 200, "%d px");
						if (ImGui::IsItemActive())
							g_Config.g_Aimbot.iRangeStyle ? ImGui::GetOverlayDrawList()->AddCircle({ pAimbot->vecCrosshair.fX, pAimbot->vecCrosshair.fY }, g_Config.g_Aimbot.iAimbotConfig[i.first][SRANGE] * 1.5f, (ImColor)g_Config.g_Aimbot.colorRange, 64, g_Config.g_Aimbot.fOutlineThickness2) :
							ImGui::GetOverlayDrawList()->AddCircleFilled({ pAimbot->vecCrosshair.fX, pAimbot->vecCrosshair.fY }, g_Config.g_Aimbot.iAimbotConfig[i.first][SRANGE] * 1.5f, (ImColor)g_Config.g_Aimbot.colorRange, 64);
