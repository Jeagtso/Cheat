...
CConfig::CConfig(const char* szName)
{
    ...
    for (int i = 22; i <= 34; i++)
    {
        ...
        g_Config.g_Aimbot.iAimbotConfig[i][SRANGE] = 1;
...
static void to_json(json& j, const stConfig::stAimbot& o, const stConfig::stAimbot& dummy = {})
{
    ...
    for (int i = 22; i <= 34; i++)
    {
        ...
        WRITE(mapWeapons[i].szName + " SRange", iAimbotConfig[i][SRANGE]);
    ...
    WRITE("bTriggerKeyMMB", bTriggerKeyMMB);
    WRITE("bGTS", bGTS);
...
static void from_json(const json& j, stConfig::stAimbot& r)
{
    ...
    for (int i = 22; i <= 34; i++)
    {
        ...
        read(j, (mapWeapons[i].szName + " SRange").c_str(), r.iAimbotConfig[i][SRANGE]);
    ...
    read(j, "bTriggerKeyMMB", r.bTriggerKeyMMB);
    read(j, "bGTS", r.bGTS);
